...
BOOL CreateGLWindow(char* title, int width, int height, int bits, bool fullscreenflag)
{
	...
	gl->Reshape(width, height);					// 设置透视 GL 屏幕

	if (!gl->Init())									// 初始化新建的GL窗口
	{
		KillGLWindow();								// 重置显示区
							...
							LPARAM	lParam)			// 附加的消息内容
{
	switch (uMsg)									// 检查Windows消息
	{
		...
		{
			gl->Reshape(LOWORD(lParam),HIWORD(lParam));  // LoWord=Width,HiWord=Height
			return 0;								// 返回
		}
	}
					...
					int			nCmdShow)			// 窗口显示状态
{
	...
	// 创建OpenGL窗口
	if (!CreateGLWindow("NeHe's OpenGL 程序框架",800,600,16,fullscreen))
	{
		return 0;									// 失败退出
	}
	...
	while(!done)									// 保持循环直到 done=TRUE
	{
		...
		else										// 如果没有消息
		{
			...
			if (keys[VK_F1])						// F1键按下了么?
			{
				...
				// 重建 OpenGL 窗口
				if (!CreateGLWindow("OpenGL Fog Demo",640,480,16,fullscreen))
				{
